...
class BaseLayerWidget extends StatefulWidget {
  BaseLayerWidget({
    Key? key,
    required this.base,
    required this.size,
  ...
  @override
  State<BaseLayerWidget> createState() => _BaseLayerWidgetState();
}

class _BaseLayerWidgetState extends State<BaseLayerWidget> {
  late Size size;
  late BoxConstraints constraints;
  double angle = 0;
  Offset position = Offset.zero;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Stack(
          children: <Widget>[
            if (selectedAssetId == backgroundKey)
              ..._buildCenterLine(constraints, isCenteredHorizontally, isCenteredVertically),
            Positioned(
              top: position.dy,
              left: position.dx,
